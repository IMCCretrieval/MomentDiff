...
from momentdiff.span_utils import span_xx_to_cxw
...
class StartEndDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.load_labels:
            if self.dset_name == 'activity' or self.dset_name == 'tacos':
                model_inputs["span_labels"] = self.get_anet_span_labels(meta["relevant_windows"], meta["duration"], ctx_l)  # (#windows, 2)
                model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs["saliency_all_labels"] = \
                    self.get_saliency_labels_anet(meta["relevant_windows"][0], meta["duration"],ctx_l)  # only one gt
            else:
    ...
    def get_saliency_labels_anet(self, gt_window,duration, ctx_l, max_n=2):
        # scale =  int(ctx_l/aaa)  ctx_l =100  duration=10s  gt_window=[3,6]
        # print(scale) clip_len = 0.1 gt_st = 30 gt_ed = 60 
    ...
    def get_anet_span_labels(self, windows, duration, ctx_l):
        """
    ...
    def avgfeats(self, feats, num_pre_clips):
        ...
        if num_src_clips >= num_pre_clips:
            idxs = idxs.round().long().clamp(max=num_src_clips-1)
        else:
            idxs = idxs.floor().long().clamp(max=num_src_clips-1)
    ...
    def _get_video_feat_by_vid(self, vid):
        ...
        if self.dset_name == 'charades_vgg':
            v_feat_list = []
        ...
        elif self.dset_name == 'activity' or self.dset_name == 'tacos' :
            v_feat_list = []
            true_feat_len = 100
            for _feat_dir in self.v_feat_dirs:
                _feat_path = join(_feat_dir, f"{vid}.npz")
                _feat = np.load(_feat_path)["arr"]
                feat = F.normalize(torch.from_numpy(_feat),dim=1)

                _feat = self.avgfeats(feat, true_feat_len).numpy()
