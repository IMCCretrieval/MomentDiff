...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        tgt_ids = torch.full([len(tgt_spans)], self.foreground_label)   # [total #spans in the batch] self.foreground_label=0
...
def get_list(arr):
    list_of_tuples = []
    for val in arr:
        temp_arr_indices = np.arange(val)
        list_of_tuples.append((temp_arr_indices, temp_arr_indices))
