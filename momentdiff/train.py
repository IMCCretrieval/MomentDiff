...
from momentdiff.config import BaseOptions
from momentdiff.start_end_dataset import \
...
from momentdiff.inference import eval_epoch, start_inference, setup_model
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        outputs = model(targets, **model_inputs) 
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    with torch.no_grad():
        metrics_no_nms, metrics_nms, eval_loss_meters, latest_file_paths = \
            eval_epoch(model, val_dataset, opt, save_submission_filename, 0, criterion, tb_writer)
    # # log
