...
from momentdiff.span_utils import generalized_temporal_iou, span_cxw_to_xx

from momentdiff.matcher import build_matcher
from momentdiff.transformer import build_transformer
from momentdiff.position_encoding import build_position_encoding
from momentdiff.misc import accuracy
import numpy as np
...
class MomentDiff(nn.Module):
    """ This is the MomentDiff module that performs moment localization. """

    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
                 num_queries, input_dropout, aux_loss=False,
                 contrastive_align_loss=False, contrastive_hdim=128,
        ...
        Parameters:
            ...
            num_queries: number of object queries, ie detection slot. This is the maximal number of objects
                         MomentDiff can detect in a single video.
        ...
        self.contrastive_align_projection_txt = nn.Linear(hidden_dim, contrastive_hdim) #set_128
        self.contrastive_align_projection_vid = nn.Linear(hidden_dim, 1) 
    ...
    def forward(self, gt_target, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        src_vid = self.input_vid_proj(src_vid) #torch.Size([32, 75, 2818]) -> 256
        src_txt = self.input_txt_proj(src_txt) #torch.Size([32, 22, 512]) -> 256
        src = torch.cat([src_vid, src_txt], dim=1)  # (bsz, L_vid+L_txt, d) torch.Size([32, 97, 256])
        mask = torch.cat([src_vid_mask, src_txt_mask], dim=1).bool()  # (bsz, L_vid+L_txt) torch.Size([32, 97])
        pos_vid = self.position_embed(src_vid, src_vid_mask)  # (bsz, L_vid, d)
        pos_txt = self.txt_position_embed(src_txt) if self.use_txt_pos else torch.zeros_like(src_txt)  # (bsz, L_txt, d)
        # pad zeros for txt positions
        pos = torch.cat([pos_vid, pos_txt], dim=1) #torch.Size([32, 97, 256])
        # (#layers, bsz, #queries, d), (bsz, L_vid+L_txt, d)

        # for global token
        mask_ = torch.tensor([[True]]).to(mask.device).repeat(mask.shape[0], 1) #torch.Size([32, 1])
        ...
        outputs_class, outputs_coord, hs, memory, memory_global = self.transformer(src, ~mask, pos, gt_target, video_length=video_length)
        ...
        if self.contrastive_align_loss:
            ...
            proj_vid_mem =  self.contrastive_align_projection_vid(vid_mem) 
            out.update(dict(
        ...
        _, _, _, memory_neg, memory_global_neg = self.transformer(src_neg, ~mask_neg,  pos_neg, gt_target, video_length=video_length)
        vid_mem_neg = memory_neg[:, :src_vid.shape[1]]
        out["saliency_scores"] = self.contrastive_align_projection_vid(vid_mem) 
        out["saliency_scores_neg"] = self.contrastive_align_projection_vid(vid_mem_neg) #分类正负样本
...
class SetCriterion(nn.Module):
    ...
    def loss_sim(self, outputs, targets, indices, log=True):
        ...
        # Neg pair loss
        saliency_scores_neg = outputs["saliency_scores_neg"].clone().squeeze()    #..squeeze()  # (N, L)
        loss_neg_pair = (- torch.log(1. - torch.sigmoid(saliency_scores_neg)) * vid_token_mask).sum(dim=1).mean()

        saliency_scores = outputs["saliency_scores"].clone().squeeze()    #.squeeze()  # (N, L)
        saliency_contrast_label = targets["saliency_all_labels"]
        criterion = nn.BCELoss()
        loss_bce = criterion(torch.sigmoid(saliency_scores)*vid_token_mask,
                        saliency_contrast_label[:, 0:saliency_scores.shape[1]]*vid_token_mask)
        ...
        pos_mask = (saliency_contrast_label >= 1)  # positive when equal or higher than rand_idx=1
        ...
        saliency_scores = outputs["saliency_scores"].squeeze()   # (N, L)
        ...
        #anet or anti len or anti mom
        # loss_bce = loss_bce - 0.1 * (mean_log_prob_pos * batch_drop_mask).mean()
        # loss_sim = torch.clamp(self.saliency_margin + neg_scores - pos_scores, min=0).sum() \
        #         / (len(pos_scores) * num_pairs) * 2  # * 2 to keep the loss the same scale
        # loss_sim = 10 * loss_sim + 10 * loss_bce 

        #charades
        loss_bce = loss_bce - (mean_log_prob_pos * batch_drop_mask).mean()
        loss_sim = torch.clamp(self.saliency_margin + neg_scores - pos_scores, min=0).sum() \
                        / (len(pos_scores) * num_pairs) * 2  # * 2 to keep the loss the same scale
        loss_sim = loss_sim + loss_bce 

        return {"loss_sim": loss_sim}
...
def build_model(args):
    ...
    if args.aux_loss:
        ...
        for i in range(args.dec_layers - 1):
            aux_weight_dict.update({k + f'_{i}': v for k, v in weight_dict.items() if k != "loss_sim"})
